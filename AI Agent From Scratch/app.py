...
# from agents.summary_validator_agent import SummarizeValidatorAgent

def main():
    st.set_page_config(page_title="Multi-Agent Medical AI System", layout="wide")
    ...
    task = st.sidebar.selectbox("Choose a task:", [
        ...
        "Sanitize Medical Data Personal Health Information (PHI)"
    ])

    agent_manager = AgentManager(max_retrieves=2, verbose=True)
    ...
    elif task == "Sanitize Medical Data Personal Health Information (PHI)":
...
def summarize_section(agent_manager):
    ...
    if st.button("Summarize"):
        if text:
            ...
            validator_agent = agent_manager.get_agent("summarize_validation")
...
def sanitize_data_section(agent_manager):
    ...
    if st.button("Sanitize Data"):
        if medical_data:
            ...
            validator_agent = agent_manager.get_agent("sanitize_validator")
